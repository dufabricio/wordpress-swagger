...
function handleLogin() {
  ...
  popupDialog = jQuery(
    [
      ...
      '</div>',
      '</div>'].join(''));
  jQuery(document.body).append(popupDialog);
  ...
  popupDialog.find('button.api-popup-authbtn').click(function() {
    ...
    var scopes = []
    var o = jQuery('.api-popup-scopes').find('input:checked');

    for(k =0; k < o.length; k++) {
      scopes.push(jQuery(o[k]).attr('scope'));
    }
...
function onOAuthComplete(token) {
  ...
  if(token) {
    if(token.error) {
      var checkbox = jQuery('input[type=checkbox],.secured')
      ...
      if(b){
        ...
        $.each(jQuery('.auth #api_information_panel'), function(k, v) {
          ...
          if(children && children.childNodes) {
            ...
            if(diff.length > 0){
              o = v.parentNode;
              jQuery(o.parentNode).find('.api-ic.ic-on').addClass('ic-off');
              jQuery(o.parentNode).find('.api-ic.ic-on').removeClass('ic-on');

              // sorry, not all scopes are satisfied
              jQuery(o).find('.api-ic').addClass('ic-warning');
